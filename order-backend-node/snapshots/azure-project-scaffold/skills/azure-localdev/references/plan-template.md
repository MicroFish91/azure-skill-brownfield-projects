...
Create `.azure/local-dev.plan.md` using this template. This file is **mandatory** and serves as the source of truth for the entire local development setup workflow.
...
> Generated by the `azure-localdev` skill. This plan describes the local
> development setup requirements for the current workspace.
>
> **Status:** Planning | Approved | Executing | Ready  
...
8. **Track status** — Update the **Status** field at the top as you progress
...
10. **Update Last Updated timestamp** — Set **Last Updated** to the current UTC datetime in ISO 8601 format every time the Status field changes (Planning → Approved → Executing → Ready) or any other edit is made to the plan.
